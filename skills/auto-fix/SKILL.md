...
| CUDA graph mode or `use_inductor_graph_partition` differ. |  make all  numerically relevant settings the same (model, attention backend, parallelism, deterministic sampling etc.) | Strong Contract #4 |
| Cross-runtime (vLLM vs HF), eager vs compile, other compilation strategies or fused passes, TP/PP/EP, DCP degree, prompt vs prompt_embeds | **Tolerance oracle** (§1) | c1 fails — oracle now tolerates drift |
...
**BI is the preferred first choice whenever it applies.** It defeats criterion 3 by installing a real bitwise contract, which is a stronger fix than tolerance (which only defeats criterion 1 by accepting drift). Reach for §1 tolerance only when the differing axis is *structural* — a different runtime, compile strategy, or parallelism topology/degree — not when it's pure batch composition/geometry under an otherwise-identical engine and strategy. CUDA graph mode and `use_inductor_graph_partition` are exceptions when they are the only numerically relevant compilation differences: preserve the exact-output oracle under Strong Contract #4. BI mode does **not** rescue a cross-runtime, cross-compile-strategy, or cross-parallelism-degree comparison (§2 caveat) — don't reach for it there even though it's the general first choice.
...
Replace exact text/token equality with a tolerance helper. This is the default fix for generated-output comparisons and the **only** correct fix for cross-runtime (vLLM vs HF) comparisons — nothing requires two different kernel stacks to be text-identical. Do not apply it to a comparison whose only numerically relevant compilation differences are CUDA graph mode or `use_inductor_graph_partition`; those are covered by Strong Contract #4.
...
**Caveat — BI does not cross runtimes, compile strategies, or parallelism degrees.** It constrains vLLM's own kernel selection and accumulation order within one fixed execution strategy. It cannot make HuggingFace match vLLM, and by itself does not license exact equality across different compile/graph-partition/fused-distributed strategies or different TP/PP/EP/DCP degrees. Pure CUDA graph mode and `use_inductor_graph_partition` differences are handled separately by Strong Contract #4 when the other relevant axes are fixed. Different parallelism topologies use different reduction trees and communication patterns, which BI does not unify. For the remaining structural axes use §1.
...
1. **Re-audit.** Re-apply the 3 criteria. The fix must make c1 **no** (oracle tolerates drift / is pinned / threshold principled) or c3 cite a now-valid clause, including clause 4 for a CUDA-graph/Inductor-partition-only comparison. If all three still hold, it's not fixed.
...
- Replacing the exact oracle with tolerance when the only numerically relevant compilation differences are `cudagraph_mode` or `use_inductor_graph_partition` under Strong Contract #4.
