...
When more than one axis differs (e.g. cross-runtime **and** prompt_embeds), remediate the weakest oracle; BI mode does **not** rescue a cross-runtime comparison (§2 caveat).
...
For same-engine paths that *should* be bitwise-identical but aren't guaranteed without it — restoration (clause 5), streaming vs non-streaming (clause 6), duplicate-in-batch (clause 7), spec decode (clause 9). Force batch-invariant kernels on **both** compared paths, then exact equality becomes a real contract.
...
**Consistency rule (from the contract):** if your fix cites clause 5/6/7/9, `batch_invariant_enabled` must actually be `true` end-to-end. Setting the var in the parent while generation happens in an unset child is a *non-fix* — Phase 2 will RECLASSIFY it.

**Caveat — BI does not cross runtimes or compile strategies.** It constrains vLLM's own kernel selection and accumulation order. It cannot make HuggingFace match vLLM, and does not license exact equality across different compile/graph-partition/fused-distributed strategies (contract "Not Strong By Default" 1–5). For those axes use §1.
...
2. **Consistency check.** If you cite clause 5/6/7/9, confirm `VLLM_BATCH_INVARIANT=1` reaches the generation process (§2). Otherwise the citation is invalid.
